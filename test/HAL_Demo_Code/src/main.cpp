...
void Image_capture(OV2640 *camera, Imager *imager){
    camera->Capture(500);
    imager->Transmit(camera->Image_data());

    // Turn Off LED same as D13 have to comment out
...
int main(void){
    ...
    // Initialize camera
    OV2640 *camera = new OV2640(hi2c1, hspi1, SPI_camera::Chip_select_pin{ GPIOB, GPIO_PIN_1 });
    HAL_Delay(500);
...
static void MX_GPIO_Init(void){
    ...
    HAL_GPIO_WritePin(GPIOB, CAM_CS_Pin, GPIO_PIN_SET);
